          ...
          {progress && (
            <div className="mt-6 bg-green-50 border border-green-100 p-5 rounded-2xl flex items-center justify-between">
              <div>
                ...
                <p className="text-green-600 font-black text-xl">Точность: {progress.completedModules.keyboard.accuracy}% | WPM: {progress.completedModules.keyboard.currentLevel * 15}</p>
              </div>
      ...
      <div className="container mx-auto p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {KEYBOARD_LEVELS.map((level) => {
          ...
          return (
            <div
              key={level.id}
              className={`bg-white rounded-4xl shadow-xl overflow-hidden flex flex-col h-full border border-slate-100 transition-all ${status === 'locked' ? 'opacity-60 grayscale' : 'hover:-translate-y-2 hover:shadow-2xl'}`}
            >
              <div className={`h-32 p-6 flex items-start justify-between ${status === 'completed' ? 'bg-green-500' : status === 'locked' ? 'bg-slate-400' : 'bg-green-600'}`}>
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all mt-8 ${status === 'locked' ? 'bg-slate-100 text-slate-400' : 'bg-green-600 text-white shadow-lg shadow-green-100 hover:bg-green-700 active:scale-95'}`}
                >
                  {status === 'locked' ? 'Заблокировано' : status === 'completed' ? 'Повторить' : 'Начать'}
                </button>
              </div>
            </div>
          );
